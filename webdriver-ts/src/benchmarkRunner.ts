...
async function performRetryableRun(
  ...
  for (; retry <= 5 && attemptRetry; retry++) {
    ...
    warnings = [];
    console.log(
      `Executing benchmark ${framework.name} and benchmark ${benchmark.id} retry # ${retry}`
    );
    ...
    let benchMsg: any = await forkedRun(
      runFrameworks,
      framework.name,
      framework.keyed,
      benchmark.id,
    ...
    if (benchMsg.failure) {
      console.log(
        `Executing ${framework.uri} and benchmark ${benchmark.id} failed with a technical error: ${benchMsg.failure}`
      );
      errors.push(
        `Executing ${framework.uri} and benchmark ${benchmark.id} failed with a technical error: ${benchMsg.failure}`
      );
      if (config.EXIT_ON_ERROR)
        throw "Exiting because of an technical error and config.EXIT_ON_ERROR = true";
    } else {
      let errorsAndWarnings = benchMsg as ErrorAndWarning;
      if (errorsAndWarnings.error)
        errors.push(
          `Executing ${framework.uri} and benchmark ${benchmark.id} failed: ` +
      ...
      if (
        errorsAndWarnings.error &&
        errorsAndWarnings.error.indexOf(
      ...
      if (errorsAndWarnings.error && config.EXIT_ON_ERROR)
        throw "Exiting because of an error and config.EXIT_ON_ERROR = true";
    }
  }
...
async function runBench(
  ...
  let runBenchmarks = benchmarks.filter((b) =>
    benchmarkNames.some((name) => b.id.toLowerCase().indexOf(name) > -1)
  );

  let restart: string = undefined;
  let index = runFrameworks.findIndex(
    (f) => f.fullNameWithKeyedAndVersion === restart
  );
  if (index > -1) {
...
async function main() {
  let runBenchmarks = (args.benchmark && args.benchmark.length > 0
    ? args.benchmark
    : [""]
  ).map((v) => v.toString());
  ...
  if (args.installed) {
    ...
    console.log(
      "MODE: Directory names. Using arguments as the directory names to be re-run: ",
      allArgs
    );
    let matchesDirectoryArg = (dir: string) => {
      return (
        allArgs.length == 0 ||
        allArgs.some((pattern) => {
...
=======
    ...
    console.log("MODE: Classic command line options");
    let frameworkNames = (args.framework && args.framework.length > 0
      ? args.framework
      : [""]
    ).map((v) => v.toString());
    let frameworks = await initializeFrameworks();
    runFrameworks = frameworks.filter((f) =>
      frameworkNames.some(
        (name) => f.fullNameWithKeyedAndVersion.indexOf(name) > -1
      )
    );
  }
  let count = Number(args.count);
