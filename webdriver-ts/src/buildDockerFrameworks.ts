...
async function prepareDockerVolume() {
  ...
  try {
    ...
    if (e.message.indexOf("No such volume: js-framework-benchmark") > -1) {
      console.log(
        "Volume not found. Creating volume: docker volume create js-framework-benchmark"
      );
...
async function stopContainerIfRunnning() {
  ...
  if (res.indexOf("js-framework-benchmark") > -1) {
    console.log(
      "js-framework-benchmark container runs. Stopping this container."
    );
...
async function startDocker() {
  ...
  exec(
    "docker run --rm -d -i --name js-framework-benchmark -p 8080:8080 --volume js-framework-benchmark:/build js-framework-benchmark-centos",
    {
      stdio: "inherit",
    }
  );
...
async function copyFilesToDocker() {
  try {
    ...
    console.log(
      "copy files to docker failed. Trying to stop container js-framework-benchmark"
    );
...
async function runBuildInDocker() {
  ...
  exec(
    "docker exec -it -w /build js-framework-benchmark node build.js --benchmarks_only",
    {
      stdio: "inherit",
    }
  );
