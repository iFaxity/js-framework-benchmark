    ...
    allBenchmarks.forEach((benchmarkInfo) => {
      ...
      if (fs.existsSync(file)) {
        ...
        if (
          ...
          resultNice.values.length != config.REPEAT_RUN
        ) {
          console.log(
            `WARNING: for ${framework.uri} and benchmark ${benchmarkInfo.id} count was ${resultNice.values.length}. We expected ${config.REPEAT_RUN}`
          );
        } else if (
          benchmarkInfo.type === BenchmarkType.MEM &&
          resultNice.values.length != config.REPEAT_RUN_MEM
        ) {
          console.log(
            `WARNING: for ${framework.uri} and benchmark ${benchmarkInfo.id} count was ${resultNice.values.length}. We expected ${config.REPEAT_RUN_MEM}`
          );
        } else if (
          benchmarkInfo.type === BenchmarkType.STARTUP &&
          resultNice.values.length != config.REPEAT_RUN_STARTUP
        ) {
          console.log(
            `WARNING: for ${framework.uri} and benchmark ${benchmarkInfo.id} count was ${resultNice.values.length}. We expected ${config.REPEAT_RUN_STARTUP}`
          );
        }
      } else {
