  ...
  try {
    ...
    try {
      results = await lighthouse(
        `http://localhost:${benchmarkOptions.port}/${framework.uri}/index.html`,
        opts,
        null
      );
      await chrome.kill();
...
async function computeResultsCPU(
  ...
  if (results.length !== benchmarkOptions.numIterationsForCPUBenchmarks) {
    console.log(
      `soundness check failed. number or results isn't ${benchmarkOptions.numIterationsForCPUBenchmarks}`,
      results,
...
async function computeResultsMEM(
  ...
  if (results.length !== 1) {
    ...
    console.log(
      `soundness check failed. number or results isn't 1*`,
      results,
      asString(filteredEvents)
    );
...
function writeResult<T>(res: Result<T>) {
  ...
  for (let resultKind of benchmark.resultKinds()) {
    ...
    fs.writeFileSync(
      `${config.RESULTS_DIRECTORY}/${fileName(res.framework, resultKind)}`,
...
async function runMemBenchmark(
  ...
  try {
    for (let i = 0; i < benchmarkOptions.numIterationsForMemBenchmarks; i++) {
      ...
      await afterBenchmark(driver, benchmark, framework);
      await driver.executeScript("console.timeStamp('afterBenchmark')");
      let result = await computeResultsMEM(
        driver,
        benchmarkOptions,
        framework,
        benchmark,
      ...
      if (config.LOG_DETAILS)
        console.log(
          "comparison of memory usage. GC log:",
          result,
          " :takeHeapSnapshot",
          snapshotSize
        );
...
async function runStartupBenchmark(
  ...
  try {
    for (
      let i = 0;
      i < benchmarkOptions.numIterationsForStartupBenchmark;
      i++
    ) {
...
export async function executeBenchmark(
  ...
  let runFrameworks = frameworks
    .filter((f) => f.keyed === keyed)
    .filter((f) => frameworkName === f.name);
  ...
  if (benchmark.type == BenchmarkType.STARTUP) {
    errorAndWarnings = await runStartupBenchmark(
      framework,
      benchmark,
      benchmarkOptions
    );
...
export async function performBenchmark(
  frameworks: FrameworkData[],
  keyed: boolean,
  frameworkName: string,
  benchmarkName: string,
  ...
  let errorAndWarnings = await executeBenchmark(
    frameworks,
    keyed,
    frameworkName,
    benchmarkName,
