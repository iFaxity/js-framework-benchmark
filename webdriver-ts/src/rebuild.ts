  ...
  if (frameworks.length === 0) {
    ...
    for (let framework of frameworks) {
      ...
      if (!fs.existsSync(dir)) throw "ERROR: directory " + dir + " not found";
        ...
        console.log(
          "running rm -rf package-lock.json yarn.lock dist elm-stuff bower_components node_modules"
        );
        try {
          exec(
            "rm -rf package-lock.json yarn.lock dist elm-stuff bower_components node_modules",
            {
              cwd: dir,
              stdio: "inherit",
            }
    ...
    console.log(
      "npm run bench -- --headless --noResults --exitOnError true --count 1  " +
        frameworkNames
    );
    exec(
      "npm run bench -- --headless --noResults --exitOnError true --count 1 " +
    ...
    console.log(
      "======> Please rerun the benchmark: npm run bench ",
      frameworkNames
    );
  }
}
